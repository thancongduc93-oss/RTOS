...
# ----- PID -----
PID_KP = 1.5           # Giam Kp de giam xoay qua da
PID_KI = 0.02          # Giam Ki tranh integral windup
PID_KD = 0.8
PID_INTEGRAL_MAX = 50.0
PID_OUTPUT_MAX   = 150.0

# ----- Heading Filter & Deadband -----
HEADING_ALPHA    = 0.2   # EMA filter heading: 0.0=smooth, 1.0=raw (chong Gimbal Lock)
HEADING_DEADBAND = 5.0   # Sai lech < 5 do → khong re, di thang
...
def mix_motors(speed, turn):
    ...
    turn:  steering PID output (am=trai, duong=phai)

    FIX: Khi error duong (phai queo Phai):
      - turn duong → Motor Trai NHANH hon → thuyen queo PHAI → Dung!
      - (truoc day bi nguoc: left=speed-turn → queo TRAI → xoay vong)
    Returns: (left_pwm, right_pwm)
    """
    left_pwm  = speed + turn   # FIX: doi dau + (truoc la -)
    right_pwm = speed - turn   # FIX: doi dau - (truoc la +)
...
data = {
    ...
    # BNO055 tu ESP32 Motor (CAN 0x100)
    "heading": 0.0,
    "heading_filtered": 0.0,  # FIX: heading sau khi loc EMA chong Gimbal Lock
    "heading_init": False,    # FIX: co de biet lan dau nhan heading
    "roll": 0.0,
    "pitch": 0.0,
    "imu_sys_cal": 0,
    "imu_mag_cal": 0,
    ...
    "left_pwm": 0,
    "right_pwm": 0,
    "left_percent": 0,   # Feedback motor
    "right_percent": 0,  # Feedback motor
    "nav_mode": MODE_STOP,
...
def parse_can_message(msg):
    ...
    with data_lock:
        ...
        # 0x100: BNO055 IMU (heading, roll, pitch x100 + cal)
        ...
        # 0x104: Motor Feedback (left_us, right_us tu ESP)
            ...
            # Convert pwm 1000-2000 sang phan tram (0-100%)
...
def can_reader_thread():
    ...
    # DEBUG: Dem so frame nhan duoc de chan doan loi phan cung
    frame_count = 0
    id_counts = {}
    ...
    while True:
        try:
            ...
            if message is not None:
                ...
                frame_count += 1
                hex_id = f"0x{message.arbitration_id:03X}"
            ...
            if now - last_report >= 5.0:
                if frame_count > 0:
                    summary = ", ".join(f"{k}={v}" for k, v in sorted(id_counts.items()))
                    print(f"📊 [CAN-RX] {frame_count} frames trong 5s | {summary}")
                    no_data_warns = 0
                else:
                    no_data_warns += 1
                    print(f"⚠️  [CAN-RX] KHONG NHAN DUOC FRAME NAO! ({no_data_warns * 5}s)")
                    if no_data_warns == 1:
                        print("   → Kiem tra: Day CAN_H/CAN_L, nguon ESP32, MCP2515/TJA1050")
                        print("   → Thu chay: candump can0  (de xem co frame nao tren bus)")
                frame_count = 0
                id_counts = {}
                last_report = now

        except Exception as e:
            print(f"❌ [CAN-RX] Loi doc: {e}")
...
def navigation_thread():
    ...
    while True:
        ...
        with data_lock:
            ...
            heading  = data["heading_filtered"]   # FIX: Dung heading da loc
            gps_ok   = data["gps_ok"]
        ...
        # FIX Deadband: Sai lech nho hon HEADING_DEADBAND thi khong re
        # Tranh motor run rit lien tuc khi thuyen da gan dung huong
        if abs(error) < HEADING_DEADBAND:
            turn = 0.0
            pid.reset()  # Reset integral de khong tich luy
        else:
        ...
        if DEBUG_PRINT and (time.time() - last_debug_time) >= DEBUG_INTERVAL_S:
            last_debug_time = time.time()
            dir_arrow = "→R" if error > HEADING_DEADBAND else ("←L" if error < -HEADING_DEADBAND else "↑STRAIGHT")
            lock_warn = " ⚠️CAL_LOW" if imu_cal[1] < 2 else ""
            print(
                f"🧭 [NAV] AUTO | "
                ...
                f"Cal=S{imu_cal[0]}M{imu_cal[1]}{lock_warn}"
            )
