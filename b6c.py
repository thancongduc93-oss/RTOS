...
def mix_motors(speed, turn):
    ...
    Nếu turn > 0 (bẻ Phải): motor Trái giảm, motor Phải tăng (đã đảo ngược L/R)
    Nếu turn < 0 (bẻ Trái): motor Phải giảm, motor Trái tăng (đã đảo ngược L/R)
    """
    left_pwm  = speed - turn
...
def navigation_thread():
    ...
    while True:
        ...
        with data_lock:
            ...
            if target_src == "WEB" and len(data["mission_waypoints"]) > 0:
                wp_idx = data["current_wp_idx"]
                if wp_idx < len(data["mission_waypoints"]):
                    target_lat = data["mission_waypoints"][wp_idx]["lat"]
                    target_lon = data["mission_waypoints"][wp_idx]["lng"]
                else:
        ...
        # ----- Kiem tra khoang cach: < 5m → tha troi hoac chuyen waypoint -----
        if distance < COAST_DISTANCE_M:
            is_last_wp = True
            with data_lock:
                if target_src == "WEB" and data["current_wp_idx"] < len(data["mission_waypoints"]) - 1:
                    is_last_wp = False
                    data["current_wp_idx"] += 1
                    print(f"✅ Đã đến Waypoint {data['current_wp_idx']}. Chuyển hướng tới điểm tiếp theo!")
            ...
            if is_last_wp:
                ...
                # Chua phai diem cuoi, giu nguyen toc do va chi reset PID integral
                speed = web_speed
                pid.reset()
                turn_val = 0.0
                nav_mode = MODE_AUTO
            ...
            # Kiem tra giam toc: Chi giam toc neu dang gan diem CUOI CUNG hoac Vong Tay
            is_last_wp = True
            with data_lock:
                if target_src == "WEB" and data["current_wp_idx"] < len(data["mission_waypoints"]) - 1:
                    is_last_wp = False
            
            # Giam toc khi gan (< 20m) - chi ap dung cho diem cuoi
            if is_last_wp and distance < SLOWDOWN_DISTANCE:
...
def handle_command(topic, payload):
    try:
        if topic.endswith("mode"):
            ...
            with data_lock:
                data["mission_waypoints"] = mission
                data["current_wp_idx"] = 0
                if len(mission) > 0:
                    data["web_lat"] = mission[0].get("lat")
                    data["web_lon"] = mission[0].get("lng")
                    data["target_source"] = "WEB"
                    print(f"📍 [MQTT] Nhận danh sách {len(mission)} điểm đến từ Web!")
                else:
                    data["target_source"] = "BRACELET"
                    print("📍 [MQTT] Xoa điểm Web, tro ve bam theo Vong tay!")
    ...
    try:
        while True:
            ...
            if d["lat"] is not None and d["lon"] is not None:
                ...
                with data_lock:
                    if d["target_source"] == "WEB" and len(d["mission_waypoints"]) > d["current_wp_idx"]:
                        t_lat = d["mission_waypoints"][d["current_wp_idx"]]["lat"]
                        t_lon = d["mission_waypoints"][d["current_wp_idx"]]["lng"]
                    else:
                        t_lat = d["bracelet_lat"]
                        t_lon = d["bracelet_lon"]
